...
const AiSummary: React.FC<AiSummaryProps> = ({ planData, indicatorData }) => {
    ...
    const handleGenerateSummary = useCallback(async () => {
        ...
        setSummary('');
        try {
            const result = await generateSummary(planData, indicatorData);
            setSummary(result);
        } catch (e: any) {
    ...
    const renderFormattedSummary = (text: string) => {
        ...
        return sections.map((section, index) => {
            if (section.startsWith('**') && section.endsWith('**')) {
                return <h3 key={index} className="text-lg font-bold mt-4 mb-2 text-sky-500 dark:text-sky-400">{section.replace(/\*\*/g, '')}</h3>;
            }
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-6 border border-slate-200 dark:border-slate-700">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                 <div className="flex items-center gap-3">
                    <SparklesIcon className="w-7 h-7 text-sky-600 dark:text-sky-400" />
                    <h2 className="text-xl font-bold text-slate-900 dark:text-white">ملخص وتحليل Gemini</h2>
                </div>
                <button
                    onClick={handleGenerateSummary}
                    disabled={isLoading}
                    className="flex items-center justify-center gap-2 px-4 py-2 bg-sky-600 text-white font-semibold rounded-lg shadow-md hover:bg-sky-700 disabled:bg-slate-400 disabled:cursor-not-allowed transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 dark:focus:ring-offset-slate-800"
                >
                    {isLoading ? (
                        <>
            ...
            {summary && !isLoading && (
                <div className="prose prose-slate dark:prose-invert max-w-none mt-4 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg whitespace-pre-wrap">
